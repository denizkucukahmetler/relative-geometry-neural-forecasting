...
MODELS   = ["mlp", "transformer","rnnautoreg"]
DATASETS = ["random_skew","lorenz","double_pendulum","spiral","hopf","logistic_map"] # pod is not added since it's data is not generated in our pipeline 
SEEDS    = range(1,2)
TAG      = "best"
...
for seed in SEEDS:
    seed_all(seed)
    for ds in DATASETS:
        for noise in [0,0.01,0.05,0.1,0.2,0.3,0.6,0.8]:
            
            cfg = ConfigManager(ds, dev.type)
            dc = cfg.get_current_dataset_config()
            dc["noise_pct"] = noise      
            ...
            for model in MODELS:
                ...
                elif model == "transformer": #CHECKED
                ...
                elif model == "koopmantransformer": #CHECKED
                    dm  =int(pick(p,"dm","d_model",default=256))
                    nL  =int(pick(p,"L","n_layers","nl",default=2))
                ...
                elif model == "koopmanrnnautoreg":  #CHECKED
                ...
                elif model == "nodetransformer": #CHECKED
                    dm  =int(pick(p,"dm","d_model",default=256))
                    nL  =int(pick(p,"L","n_layers","nl",default=2))
                    ode =int(pick(p,"ode","ode_hd","oh",default=128))
                    do  =float(pick(p,"do","dropout",default=0.1))
                ...
                elif model == "nodernnautoreg":  #CHECKED
                    hd  =int(pick(p,"hd","hid","hidden_dim",default=128))
                    nL  =int(pick(p,"L","layers","num_layers",default=2))
                    ode =int(pick(p,"ode","ode_hd","oh",default=128))
            ...
            exp = f"{ds}_{TAG}_seed{seed}_noise{noise}"
            ...
            experiment_manager = ExperimentManager(cfg, device)

            models, latent_spaces, absolute_latent_spaces = experiment_manager.evalute_latent_spaces_exp(exp, points_to_embed, anchors, random_anchors=True)

            experiment_manager.benchmark_models_exp(exp)